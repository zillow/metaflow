...
@click.option(
    ...
    "user_namespace",
    default=None,
    help="Change the Metaflow namespace from the default to the given Metaflow namespace.",
    show_default=True,
)
@click.option(
    "--k8s-namespace",
...
def run(
    ...
    **kwargs,
    ...
    if yaml_only:
        ...
        metaflow_run_id = to_metaflow_run_id(argo_run_id)
        metaflow_ui_url = run_id_to_metaflow_url(flow.name, argo_run_id)
        argo_ui_url = run_id_to_url(argo_run_id, kubernetes_namespace)
        # ddog_ui_url = (
...
def create(
    ...
    if yaml_only:
        ...
        workflow, path = flow.create_workflow_yaml_file(
            pipeline_path, output_format="argo-workflow-template", name=workflow_name
        )
        obj.echo(
...
def trigger(
    ...
    **kwargs,
    ...
    workflow_name: str = resolve_workflow_name(name)
    workflow: Dict[str, Any] = KubeflowPipelines.trigger(
        kubernetes_namespace, workflow_name, params
    )
    argo_run_id = workflow["metadata"]["name"]
    metaflow_run_id = to_metaflow_run_id(argo_run_id)

    if run_id_file:
        with open(run_id_file, "w") as f:
            f.write(str(argo_run_id))

    obj.echo(
        f"Workflow *{workflow_name}* triggered on Argo Workflows "
        f"(Argo run-id *{argo_run_id}*).",
        bold=True,
    )

    metaflow_ui_url = run_id_to_metaflow_url(current.flow_name, argo_run_id)
    argo_ui_url = run_id_to_url(argo_run_id, kubernetes_namespace)
    ...
    argo_workflow_name = workflow["metadata"]["name"]
